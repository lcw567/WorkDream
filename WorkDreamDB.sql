...
-- RESUME (이력서)
CREATE TABLE RESUME(        
    RESUME_NO NUMBER NOT NULL PRIMARY KEY,  -- RESUME_SEQ
    PERSON_NO NUMBER NOT NULL,
    ...
    USER_ROAD_ADDRESS VARCHAR2(255),
    USER_DETAIL_ADDRESS VARCHAR2(255),
    USER_PIC VARCHAR2(500),
    EDUCATION_LEVEL VARCHAR2(50) DEFAULT '기본값' NULL,
...
=======
COMMENT ON COLUMN RESUME.USER_NAME IS '이름';
COMMENT ON COLUMN RESUME.USER_GENDER IS '성별(남성: M / 여성: W)';
COMMENT ON COLUMN RESUME.USER_BIRTH IS '생년월일';
COMMENT ON COLUMN RESUME.USER_EMAIL IS '이메일';
COMMENT ON COLUMN RESUME.USER_PHONE IS '휴대전화';
COMMENT ON COLUMN RESUME.USER_ADDRESS IS '일반 주소';
COMMENT ON COLUMN RESUME.USER_ROAD_ADDRESS IS '도로명 주소';
COMMENT ON COLUMN RESUME.USER_DETAIL_ADDRESS IS '상세 주소';
COMMENT ON COLUMN RESUME.USER_PIC IS '증명사진';
COMMENT ON COLUMN RESUME.EDUCATION_LEVEL IS '학력 수준(educationLevel)';
COMMENT ON COLUMN RESUME.ACADEMIC_NAME_EL IS '초등학교 이름';
COMMENT ON COLUMN RESUME.ACADEMIC_NAME_MI IS '중학교 이름';
COMMENT ON COLUMN RESUME.ACADEMIC_NAME_HI IS '고등학교 이름';
COMMENT ON COLUMN RESUME.ACADEMIC_NAME IS '학력 이름';
COMMENT ON COLUMN RESUME.MAJOR IS '전공';
COMMENT ON COLUMN RESUME.DEGREE IS '학위';
COMMENT ON COLUMN RESUME.CREATE_DATE IS '이력서 생성일자';
COMMENT ON COLUMN RESUME.MODIFIED_DATE IS '마지막 수정일자';
COMMENT ON COLUMN RESUME.STATUS IS '공개 여부(공개: Y / 미공개: N)';
COMMENT ON COLUMN RESUME.DELETED IS '삭제 여부(미삭제: N / 삭제: Y)';


--
CREATE TABLE certificates (
>>>>>>> 01d9200d73e47266eb71bd2c45edc065a68043c3
...
CREATE SEQUENCE RESUME_SEQ START WITH 1 INCREMENT BY 1 NOCACHE; -- 이력서 고유키
