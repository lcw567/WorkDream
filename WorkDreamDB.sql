...
DROP TABLE PORTFOLIO_IMAGE CASCADE CONSTRAINTS;
...
DROP SEQUENCE SEQ_PORTFOLIO_IMAGE_NO;
...
CREATE TABLE PORTFOLIO (
    PORTFOLIO_NO NUMBER PRIMARY KEY NOT NULL,          -- 포트폴리오 고유번호
    USER_NO NUMBER NOT NULL,                           -- 작성자 (MEMBER 테이블 참조)
    TITLE VARCHAR2(255) NOT NULL,                      -- 포트폴리오 제목
    CONTENT CLOB NOT NULL,                             -- 포트폴리오 내용 (HTML 형식)
...
-- 컬럼 주석 추가
COMMENT ON TABLE PORTFOLIO IS '포트폴리오 정보 테이블';
COMMENT ON COLUMN PORTFOLIO.PORTFOLIO_NO IS '포트폴리오 고유번호';
COMMENT ON COLUMN PORTFOLIO.USER_NO IS '작성자 회원 번호 (MEMBER 테이블 참조)';
COMMENT ON COLUMN PORTFOLIO.TITLE IS '포트폴리오 제목';
COMMENT ON COLUMN PORTFOLIO.CONTENT IS '포트폴리오 내용 (HTML 형식)';
COMMENT ON COLUMN PORTFOLIO.CREATED_AT IS '작성일';
COMMENT ON COLUMN PORTFOLIO.UPDATED_AT IS '수정일';

CREATE SEQUENCE SEQ_PORTFOLIO_NO START WITH 1 INCREMENT BY 1 NOCACHE NOCYCLE;--시퀀스

-- trg_portfolio_no 트리거 생성
CREATE OR REPLACE TRIGGER trg_portfolio_no
BEFORE INSERT ON PORTFOLIO
FOR EACH ROW
BEGIN
    IF :NEW.PORTFOLIO_NO IS NULL THEN
        :NEW.PORTFOLIO_NO := SEQ_PORTFOLIO_NO.NEXTVAL;
    END IF;
END;
/
-- ==============================================================================
-- 2. PORTFOLIO_IMAGE 테이블 생성
-- ==============================================================================

-- PORTFOLIO_IMAGE 테이블 생성
CREATE TABLE PORTFOLIO_IMAGE (
    IMAGE_NO NUMBER PRIMARY KEY NOT NULL,              -- 이미지 고유번호
    PORTFOLIO_NO NUMBER NOT NULL,                      -- 포트폴리오 번호 (PORTFOLIO 테이블 참조)
    IMAGE_URL VARCHAR2(500) NOT NULL,                  -- 이미지 URL
    DESCRIPTION VARCHAR2(255) NULL,                    -- 이미지 설명
    UPLOADED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP    -- 업로드 일시
);

-- 외래 키 제약 조건 추가
ALTER TABLE PORTFOLIO_IMAGE
ADD CONSTRAINT FK_PORTFOLIO_IMAGE_PORTFOLIO
FOREIGN KEY (PORTFOLIO_NO) REFERENCES PORTFOLIO (PORTFOLIO_NO)
ON DELETE CASCADE;

-- 컬럼 주석 추가
COMMENT ON TABLE PORTFOLIO_IMAGE IS '포트폴리오 이미지 관리 테이블';
COMMENT ON COLUMN PORTFOLIO_IMAGE.IMAGE_NO IS '이미지 고유번호';
COMMENT ON COLUMN PORTFOLIO_IMAGE.PORTFOLIO_NO IS '포트폴리오 번호 (PORTFOLIO 테이블 참조)';
COMMENT ON COLUMN PORTFOLIO_IMAGE.IMAGE_URL IS '이미지 URL';
COMMENT ON COLUMN PORTFOLIO_IMAGE.DESCRIPTION IS '이미지 설명';
COMMENT ON COLUMN PORTFOLIO_IMAGE.UPLOADED_AT IS '업로드 일시';

CREATE SEQUENCE SEQ_PORTFOLIO_IMAGE_NO START WITH 1 INCREMENT BY 1 NOCACHE NOCYCLE;  --시퀀스

-- trg_portfolio_image_no 트리거 생성
CREATE OR REPLACE TRIGGER trg_portfolio_image_no
BEFORE INSERT ON PORTFOLIO_IMAGE
FOR EACH ROW
BEGIN
    IF :NEW.IMAGE_NO IS NULL THEN
        :NEW.IMAGE_NO := SEQ_PORTFOLIO_IMAGE_NO.NEXTVAL;
    END IF;
END;
/
-- ==============================================================================
